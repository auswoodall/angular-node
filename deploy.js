...
function cloneRepo() {
	...
	cmd.get(
		'rm -rf hackathon-starter && git clone https://github.com/auswoodall/angular-node.git',
...
function transferProjectToRemote(failed, successful) {
	return ssh.putDirectory(__dirname + '/angular-node', '/home/ubuntu/angular-node-temp', {
		recursive: true,
		concurrency: 1,
...
function createRemoteTempFolder() {
	return ssh.execCommand('rm -rf angular-node-temp && mkdir angular-node-temp', { cwd:'/home/ubuntu' })
}
...
function updateRemoteApp() {
	return ssh.execCommand('cp -r angular-node-temp/* angular-node/ && rm -rf angular-node-temp/*', { cwd:'/home/ubuntu' })
}
...
function sshConnect() {
	...
	.then(function() {
		...
		console.log("Creating `angular-node-temp` folder.");
